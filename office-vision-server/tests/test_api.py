"""Server API 端到端测试：事件接收 → 幂等 → 记录生成 → 统计查询。"""
...
class TestTimezone:
    ...
    def test_抽烟记录时间输出带时区标记(self, client: TestClient) -> None:
        ...
        item = client.get("/api/smoking/records").json()["items"][0]
...
class TestSmoking:
    ...
    def test_生成抽烟记录(self, client: TestClient) -> None:
        ...
        records = client.get("/api/smoking/records").json()
    ...
    def test_今日统计(self, client: TestClient) -> None:
        ...
        summary = client.get("/api/smoking/today").json()
    ...
    def test_趋势补零(self, client: TestClient) -> None:
        ...
        trend = client.get("/api/smoking/trend?days=7").json()["days"]
        assert len(trend) == 7
